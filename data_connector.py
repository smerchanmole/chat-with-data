...
class DataCatalog:
    ...
    def connections(self) -> list[dict[str, Any]]:
        configured = []
        raw = os.getenv("CML_DATA_CONNECTIONS", "vast-data-demo:impala,go01-optimizer:hive")
        for item in filter(None, (part.strip() for part in raw.split(","))):
            name, _, engine = item.partition(":")
            configured.append(ConnectionSpec(name, engine or "impala", name). __dict__)
        ...
        if spec.get("jdbc_url"):
            ...
            for catalog in (row[0] for row in rows["rows"]):
                try:
                    ...
                    continue
            return values
        frame = self._cml_query(spec["name"], "SHOW DATABASES")
    ...
    def tables(self, spec: dict[str, Any], database: str) -> list[str]:
        ...
        frame = self._cml_query(spec["name"], query)
        return [str(row[0]) for row in frame.itertuples(index=False, name=None)]
